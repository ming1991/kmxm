...
public abstract class MonthView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(!isFirst){
            heightSize = (int) (NUM_ROWS * rowSize);
            isFirst=true;
        }
    ...
    private void drawDate(Canvas canvas,int year,int month,int startX,int startY){
        ...
        rowSize=(theme.dateHeight()*6)* 1.0F/NUM_ROWS;
    ...
    private void doClickAction(int x,int y){
        ...
        if(null == daysString || row < 0 || row > 5 || column < 0 || column >6){
            return;
        }
        setSelectDate(selYear,selMonth,daysString[row][column]);
        doClick(selYear,selMonth,daysString[row][column]);
        invalidate();
