...
public class CircleMonthView extends MonthView {
    protected int currSelectYear,currSelectMonth,currSelectDay;
    ...
    protected void drawBG(Canvas canvas, int column, int row, int year,int month,int day) {
        if(year == currSelectYear && month == currSelectMonth && day== currSelectDay){
            //绘制背景色矩形
    ...
    protected void drawText(Canvas canvas, int column, int row, int year,int month,int day) {
        ...
        boolean isAvailableDate = isAvailableDate(year,month,day);
        if (isAvailableDate){//是不是两周内的有效日期
            if(year == currSelectYear && month == currSelectMonth && day== currSelectDay){//日期为选中的日期
    ...
    //判断当前日期是否在两周时间内
    private boolean isAvailableDate(int year, int month, int day) {
        month = month + 1;
        try {
            long thisTime = DATE_FORMAT_DATE.parse(year +"-"+ month +"-"+ day).getTime();
            long twoWeekenk = System.currentTimeMillis()-15*24*60*60*1000;
            long today = System.currentTimeMillis();
            return thisTime > twoWeekenk && thisTime < today;
    ...
    @Override
    protected void doClick(int year, int month, int day) {
        if (isAvailableDate(year,month,day)){
            setCurrentSelectDate(year,month,day);
        }
    }

    public void setCurrentSelectDate(int year, int month, int day) {
        currSelectYear = year;
